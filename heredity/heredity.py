...
def update(
    probabilities: dict[
        str, dict[Literal["gene", "trait"], dict[Literal[0, 1, 2, True, False], float]]
    ],
    one_gene,
    two_genes,
    have_trait,
    ...
    for name in probabilities.keys():
        probabilities[name]["gene"][ng_of(name)] += p

        is_have_trait = name in have_trait
        probabilities[name]["trait"][is_have_trait] += p
...
def normalize(probabilities: dict[
    ...
    """
    for name in probabilities.keys():
        g = probabilities[name]["gene"]
        t = probabilities[name]["trait"]

        alpha = 1 / sum(g.values())
        for k, v in g.items():
            g[k] = v * alpha
        
        alpha = 1 / sum(t.values())
        for k, v in t.items():
            t[k] = v * alpha
